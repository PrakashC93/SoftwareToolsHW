...
#Main Execution
                                                    #an empty array to store the accelerations of all planets
...
for step in tqdm(range(NUMBER_OF_TIME_STEPS + 1)):                          #repeating for every time
    ...
    for i in range(nob):
        for j in range(nob):
            ...
            acceleration = np.append([acceleration, acc])                  #acceleration of all bodies for current time step
    acceleration = np.reshape(acceleration, (nob,2))                       #reshaping to make it homogenous with data structure
    #ac_s = acceleration                                 #appending current accelration to acc matrix
