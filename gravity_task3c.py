...
mass = np.array([4 / GRAVITATIONAL_CONSTANT, 4 / GRAVITATIONAL_CONSTANT]) #mass of planets
TIME_STEP = 0.001  # s
...
for step in tqdm(range(NUMBER_OF_TIME_STEPS + 1)):                          #repeating for every time
    ...
    temppos = pos[:,:]+(vel[:,:]*TIME_STEP)                                #Euler method to find new position and velocity
    POSI = np.dstack([POSI, temppos])
    pos = temppos
    new_vel = vel[:,:]+(acceleration[:,:]*TIME_STEP)
    vel = new_vel

#plotting traces of planet

fig, ax = plt.subplots()
ax.plot(POSI[0,0,:],POSI[0,1,:])                        #first planet traj
ax.plot(POSI[1,0,:],POSI[1,1,:])                        #second planet traj
ax.set_aspect("equal")
ax.set_xlim(-1.5, 1.5)
ax.set_ylim(-1.5, 1.5)
ax.set_title("Planet trajectories")
ax.set_xlabel("x")
ax.set_ylabel("y")
output_file_path = Path("Trajectories", "Planet_trajectories.png")
output_file_path.parent.mkdir(exist_ok=True)
fig.savefig(output_file_path)
plt.close(fig)

#plt.plot(POSI[0,0,:],POSI[0,1,:])
#plt.plot(POSI[1,0,:],POSI[1,1,:])
#plt.show()